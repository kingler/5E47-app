...
- **US-AS-5** As a resident, I can point my own **personal laptop** at the shared on-prem inference server and call the open-weight models (e.g., DeepSeek, Qwen, Llama, and the domain-specific models) directly through a standard API while on the Floor-6 network — a distinct path from the creative agents, metered and provenance-logged the same way. The models stay in the building; I can keep my own prompts and outputs.
...
- **R6 — AI Studio as a Service:** Music / Video / 3D / Animation agents wired to on-prem model stack (DeepSeek-V3 / R1, Qwen3-235B / Qwen2.5-VL, Llama 3.x / 4, plus domain-specific models — see SDP §3A.2); resident direct-endpoint access to the on-prem models; per-resident SLM fine-tune workflow on Floor 6 GPU compute; IP Catalog provenance logging; AI Studio metering, billing, and operator utilization view.
