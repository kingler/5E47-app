...
- **BR-AS-2** Each agent shall execute against the on-prem model stack: open-weight base models (DeepSeek-V3 / R1, Qwen3-235B / Qwen2.5-VL, Llama 3.x / 4) plus domain-specific models (audio: Stable Audio Open, MusicGen; video: Wan 2.2, HunyuanVideo, LTX-Video; 3D: TRELLIS, Hunyuan3D-2; animation: AnimateDiff family). No request shall reach a third-party API. The named models are **representative, not a fixed list** — they are the reasoning core ("brain") of the human-facing creative agents and are extended over time with additional domain-specific models fine-tuned to particular creative-workflow tasks. *(Canonical model identifiers and roles: SDP §3A.2.)*
...
- **BR-AS-7** Residents shall be able to connect their own **personal laptops** directly to the shared on-prem inference server's model endpoints — open-weight base models (e.g., DeepSeek, Qwen, Llama, among others) and the domain-specific models — exposed over the Floor-6 network via a standard (OpenAI-compatible) API. This is a distinct access path from the creative agents. Such access shall be on-site only (no endpoint reachable from outside Floor 6, BR-22), authenticated per resident, capability-scoped to the resident's AI-Studio tier, metered on the same GPU-hour envelope (BR-AS-5), and provenance-logged (BR-AS-4); model weights shall be served but not downloadable so they never leave the building, while a resident may retrieve their own prompts and outputs (consistent with 100% resident ownership, BR-AS-6).
