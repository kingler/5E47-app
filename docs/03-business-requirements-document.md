...
- **BR-AS-7** Residents shall be able to connect their own **personal computers** and tools directly to the shared on-prem inference server's model endpoints — open-weight base models (e.g., DeepSeek, Qwen, Llama, among others) and the domain-specific models — exposed over the Floor-6 network via a standard (OpenAI-compatible) API. This is a distinct access path from the creative agents. Such access shall be authenticated per resident, capability-scoped to the resident's AI-Studio tier, metered on the same GPU-hour envelope (BR-AS-5), provenance-logged (BR-AS-4), and confined to the quarantine boundary with no endpoint reachable from a third-party network (BR-22).
