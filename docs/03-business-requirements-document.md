...
- **BR-AS-2** Each agent shall execute against the on-prem model stack: open-weight base models (DeepSeek-V3 / R1, Qwen3-235B / Qwen2.5-VL, Llama 3.x / 4) plus domain-specific models (audio: Stable Audio Open, MusicGen; video: Wan 2.2, HunyuanVideo, LTX-Video; 3D: TRELLIS, Hunyuan3D-2; animation: AnimateDiff family). No request shall reach a third-party API. *(Canonical model identifiers and roles: SDP §3A.2.)*
...
- **BR-AS-7** Residents shall be able to connect their own workstation tools directly to the shared on-prem inference server's model endpoints — open-weight base models (DeepSeek, Qwen, Llama, among others) and the domain-specific models — exposed over the Floor-6 network via a standard (OpenAI-compatible) API. Such access shall be authenticated per resident, capability-scoped to the resident's AI-Studio tier, metered on the same GPU-hour envelope (BR-AS-5), provenance-logged (BR-AS-4), and confined to the quarantine boundary with no endpoint reachable from a third-party network (BR-22).
