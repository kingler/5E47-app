...
- **BR-AS-2** Each agent shall execute against the on-prem model stack: open-weight base models (DeepSeek-V3 / R1, Qwen 2.5 / Qwen3) plus domain-specific models (audio: Stable Audio Open, MusicGen; video: Wan 2.2, HunyuanVideo, LTX-Video; 3D: TRELLIS, Hunyuan3D-2; animation: AnimateDiff family). No request shall reach a third-party API.
