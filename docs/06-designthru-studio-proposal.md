...
- **AI Studio as a Service** stack: on-prem deployment of DeepSeek-V3 / R1, Qwen3-235B / Qwen2.5-VL, Llama 3.x / 4, and domain-specific models (audio, video, 3D, animation), served on OpenAI-compatible endpoints residents can call directly from their own tools; per-resident SLM fine-tuning pipeline on Floor-6 GPU compute; provenance into the IP Catalog; metered billing through the Finance Agent.
